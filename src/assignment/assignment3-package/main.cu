...
namespace utils {
    ...
    int verify_maximum_flow() {
        ...
        while (!q.empty()) {
            ...
            for (auto v = 0; v < utils::N; v++) {
                auto residual_cap = utils::cap[utils::idx(u, v, utils::N)] - utils::flow[utils::idx(u, v, utils::N)];
...
int main(int argc, char **argv) {
    ...
    push_relabel(num_blocks_per_grid, num_threads_per_block, utils::N, utils::src, utils::sink, cap_mat, utils::flow);
